...
**Whole-branch review follow-up — PATCH atomicity (`fix(team): atomic user patches and clearable device usernames`).** The mixed-payload guarantee was only half true: `disabled` → `role` → `password` applied in sequence, so a last-admin rejection raised by a later field left the earlier ones persisted (`{disabled:false, role:"member"}` on a store whose only admin is disabled 400'd with that admin already re-enabled). The last-admin rule is now one predicate, `findLastAdminViolation`, evaluated against the *result* of a change instead of asking "is this the last enabled admin" before it; `setUserDisabled`/`setUserRole` both route through it (unchanged behavior, single source of truth), and it is exported as `validateUserChanges(id, {disabled?, role?})` for the route to pre-flight the combined payload before applying anything. Per-mutation checks stay, so a concurrent writer between the pre-flight and the applies is still refused. The failing test builds the disabled-only-admin store by writing `users.json` directly — the API refuses to create that state, so an edited or restored file is the only way to reach it.
...
**Whole-branch review follow-up — clearable device username (same `fix(team)` commit).** `readUsername` mapped `""` to `undefined`, which the registry reads as "unchanged", so a username could be attached but never detached without deleting and re-adding the device. It is now tri-state (absent = unchanged, `""`/`null` = clear, value = set) and `UpdateDeviceInput.username` accepts `null` as the clear signal. Clearing is a sensitive change like any other credential edit: the device token is invalidated and the bare password is re-verified against the device before the change is stored, so a device that will not accept the password alone fails at edit time. An add still treats a blank username as "none"; an `auth: "none"` device only rejects *setting* a username, not clearing one.
...
**Review follow-up (identity timing + gate completeness).** The boot read/write split (store `useState` initializers reading the unscoped key before `/api/me` set the identity, while later writes went scoped) is fixed with the reviewer's option (b): an identity-keyed remount. `setActiveIdentity` now dispatches `cogpit-identity-changed` on actual transitions (with an equality guard so the post-remount useMe re-resolve cannot loop), and DeviceRoot folds `getActiveIdentity()` into its existing subtree key, so login/logout/user-switch remount App and every mount-time storage read re-runs through the scoped `deviceScopedKey`. Option (a) — an extra hold step in App's gate ladder — was not viable as written: the affected stores (usePermissions, useSessionHistory) are hooks inside App itself and run before the ladder's early returns, and holding above App would serialize personal boot (forbidden: zero new personal latency). Verified the "login/logout already unmounts the tree" claim: it holds only for the shell children (swapped against LoginScreen); App-level hooks survive those transitions, which is exactly why the remount lives in DeviceRoot. Personal edition never dispatches, so nothing remounts and boot is unchanged (pinned in DeviceRoot.test.tsx). Completed the terminal gate on the missed twins: DesktopOverlays `canOpenTerminal` (system-terminal palette action), MobileAppShell pending-view Terminal button, both keyboard shortcuts (system + integrated toggle, gated inside useKeyboardShortcuts), and ProcessPanel's `onRequestTerminal`. Minors: a raw local 401 while the edition is unknown (failed boot hello) now re-probes once and gates if the server says team; LoginScreen holds its credential fields until the edition resolves so a late team answer cannot steal focus mid-typing; the App auth-gate comment now names team-local gating.

**Whole-branch review follow-up — first-admin bootstrap screen (merge blocker, `feat(team): first-admin bootstrap screen`).** Task 9 shipped `POST /api/team/bootstrap` and Task 10 printed a URL for it, but no screen ever consumed either: the printed URL opened the team `LoginScreen`, where a founder with zero users could only get 401. Both docs described the missing page as if it existed. Landed:

- `GET /api/hello` gains `needsBootstrap` = `isTeamEdition() && isUsersStoreInitialized() && userCount() === 0` — the same predicate as authMiddleware's carve-out, so it reveals nothing that carve-out does not already imply. Personal edition and pre-init boots report `false`.
- `src/lib/auth.ts`'s cached edition probe became a cached *hello* probe: `getServerHello()` (shared, single fetch), `refreshServerHello()` (invalidate + refetch, for when the bootstrap closes), `getServerEdition()` derived from it. Reset hook renamed `__resetServerHelloForTest`.
- `src/components/BootstrapScreen.tsx` follows LoginScreen's structure and the SetupScreen full-page feel: username, optional display name, password, confirm. Client checks are only completeness + match + the 16-char minimum (both stated inline); the server's 400 strings show verbatim. Success clears the fields, refreshes the probe, then calls the same `onAuthenticated` LoginScreen uses. 426 shows the server's HTTPS message; 410 refreshes the probe and lets the gate fall through to LoginScreen.
- `useNetworkAuth` now resolves the whole handshake and exposes `needsBootstrap` + `refreshServerState`; `authChecked` waits for the handshake as well as the session check. Remote clients still start their session check immediately (gated is true for them regardless of edition), so nothing serializes.
- The App auth gate renders BootstrapScreen instead of LoginScreen when the probe says so. Personal edition reports `false` unconditionally (pinned).
- `buildTeamBootNotices` prefers `COGPIT_PUBLIC_URL`; without one it keeps the LAN URL and adds a line saying browser logins need HTTPS in front of it. `DeviceHello` in `useDevices.ts` gained the `edition`/`needsBootstrap` fields it was missing.

Verified end to end against a real team server behind a local TLS terminator (a plain-HTTP browser cannot be issued the `Secure` cookie, so HTTPS is required even on loopback): zero-user boot → BootstrapScreen → server 400 shown verbatim → founding admin created → `needsBootstrap` flips false → reload shows the team login → the new admin signs in to the full app.
...
**Docs follow-up (`docs: align team edition docs with the bootstrap flow`).** With the screen in place, both first-boot passages became true and were tightened to name it. The guide's sample banner now matches what a fresh box actually prints (LAN URL + the HTTPS reminder) with the `COGPIT_PUBLIC_URL` variant called out beside it, and both env tables note that the variable also names the first-boot address. Added the dedicated-account nudge to both hub sections — the hub machine stores that password, so a per-hub account can be rotated or disabled without cutting a person's browser session.
