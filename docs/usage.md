...
The sidebar and mobile Sessions tab show live and recent sessions, with status and attention cues. Search by project, prompt, title, branch, or pull request. Enter `#157`, a project name plus `#157`, or a GitHub pull request URL to find sessions that created or worked on that PR. Sessions are one list of cards, newest first, each showing its title, last prompt, status, branch and turns; hover a card for the full first and latest prompts and the project. The scope picker below the search box narrows the list to one project, and "Load older sessions" at the end fetches beyond the newest ones.
