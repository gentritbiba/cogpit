...
Source builds can add isolated UI plugins to the right workspace rail. Panels can react to the current project or session, expose live indicators, and preserve their state while another panel is active. The included GitHub Actions plugin shows workflow runs, jobs, and steps through the authenticated `gh` CLI. If `gh` is missing or signed out, only that panel shows setup guidance and the rest of Cogpit remains usable. See the [plugin guide](docs/plugins.md) for the typed API and registration flow. Runtime plugin installation is not supported yet.
