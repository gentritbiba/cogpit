...
describe("shouldFailClosed", () => {
  ...
  it("never fails closed where the edition signs accounts in (they replace the network password)", () => {
    expect(shouldFailClosed("0.0.0.0", false, true)).toBe(false)
    expect(shouldFailClosed("192.168.1.10", false, true)).toBe(false)
  })

  it("still fails closed when the network password signs in (regression pin)", () => {
    expect(shouldFailClosed("0.0.0.0", false, false)).toBe(true)
    expect(shouldFailClosed("192.168.1.10", false, false)).toBe(true)
  })
})
