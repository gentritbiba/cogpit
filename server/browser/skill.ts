...
export const COGPIT_BROWSER_SKILL = [
  ...
  "",
  "These instructions apply to every agent. In Cogpit, follow this",
  "skill for session ownership, login handoffs, cleanup, and waits when the",
  "general agent-browser skill gives different advice.",
  "",
  "Reuse the current browser and tab. Inspect the page once, then run a short",
  "sequence of known actions in one shell call using `&&`. Return the next",
  "observation in that same call. This avoids a model turn for every click or",
  "field while stopping the sequence if a command fails.",
  "",
  "```bash",
  "# After inspecting a form whose refs are @e1 and @e2:",
  "agent-browser fill @e1 'Jane Doe' && agent-browser fill @e2 'jane@example.com' && agent-browser snapshot -i",
  "```",
  "",
  "Use refs and selectors from observed page state. After navigation, a modal,",
  "or another structural change, end the chain with a fresh snapshot and inspect",
  "it before choosing the next action. Do not invent refs for an unseen page.",
  "If an action fails, inspect the current state before retrying; earlier actions",
  "in the chain may already have succeeded. Stop before any user handoff or",
  "approval that is still required.",
  "",
  "Prefer `snapshot -i`, a scoped snapshot, or a targeted text read for ordinary",
  "controls. Capture screenshots when layout, visual controls, or the requested",
  "verification needs them. Avoid returning both a full snapshot and screenshot",
  "after every field. Always verify the resulting state after a short sequence.",
  "",
  "Wait for a specific observed element, expected URL, or readiness condition",
  "when needed. Do not routinely add `wait --load networkidle` after `open`:",
  "background traffic can keep a usable page busy. Fixed sleeps are a last",
  "resort when no observable readiness condition is available.",
  "",
  "Keep actions within one browser session sequential. Only parallelize browser",
  "work across separate sessions when independent work is already authorized.",
  "In chains, repeat the literal `--session <name>` on every command when using",
  "a named or throwaway browser. Keep direct CLI calls visible to Cogpit's hooks;",
  "do not hide them in scripts or batch payloads. These chains work without a",
  "`batch` command or a new execution tool. `agent-browser eval` runs page",
  "JavaScript; it is not a persistent Playwright runtime.",
  "",
  "## The default browser",
  "",
