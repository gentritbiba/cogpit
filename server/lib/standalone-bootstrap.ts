...
import { homedir, type NetworkInterfaceInfo } from "node:os"
 ...
 * An edition that signs accounts in never fails closed here: every request
 * authenticates as a named user, whose credentials replace the network
 * password entirely.
 */
export function shouldFailClosed(
  host: string,
  hasNetworkPassword: boolean,
  editionOwnsSignIn = false,
): boolean {
  if (editionOwnsSignIn) return false
...
/** Where a standalone server keeps its data when nothing says otherwise. */
export function defaultDataDir(): string {
  return join(homedir(), ".config", "cogpit")
}

/** The standalone data directory: COGPIT_DATA_DIR, else the default. */
export function resolveDataDir(env: NodeJS.ProcessEnv = process.env): string {
  return env.COGPIT_DATA_DIR || defaultDataDir()
}
