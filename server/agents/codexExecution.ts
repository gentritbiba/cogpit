...
/**
 * The tier a fast-mode request asks this connection for. `null` clears a tier
 * the thread already carries; `undefined` leaves it untouched.
 *
 * The value comes from the descriptor, which records that this transport and a
 * spawned `codex exec` were given different spellings for the same request.
 */
function serviceTier(fastMode: boolean | undefined): string | null | undefined {
  if (fastMode === true) return descriptorFor("codex").serviceTier?.appServerValue ?? null
...
export function getCodexThreadIdentity(
  thread: CodexThread,
  sessionsDir = storeFor("codex").sessionsRoot() ?? "",
