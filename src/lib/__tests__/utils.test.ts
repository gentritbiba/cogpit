...
describe("getEffortOptions", () => {
  it("uses the selected model's supported reasoning efforts", () => {
    expect(getEffortOptions("claude", claude).map((option) => option.value)).toEqual([
      "low", "medium", "high", "xhigh", "max",
    ])
    expect(getEffortOptions("codex", codex, "gpt-5.6-sol").map((option) => option.value)).toEqual([
      "low", "medium", "high", "xhigh", "max", "ultra",
    ])
    expect(getEffortOptions("codex", codex, "gpt-5.6-luna").map((option) => option.value)).toEqual([
      "low", "medium", "high", "xhigh", "max",
    ])
  })
})
...
describe("normalizeEffortForAgent", () => {
  it("keeps max for claude", () => {
    expect(normalizeEffortForAgent("claude", claude, "max")).toBe("max")
  })

  it("keeps xhigh for codex", () => {
    expect(normalizeEffortForAgent("codex", codex, "xhigh")).toBe("xhigh")
  })

  it("keeps xhigh for claude now that the CLI supports it", () => {
    expect(normalizeEffortForAgent("claude", claude, "xhigh")).toBe("xhigh")
  })

  it("falls back to high for unsupported efforts", () => {
    expect(normalizeEffortForAgent("claude", claude, "bogus")).toBe("high")
  })
