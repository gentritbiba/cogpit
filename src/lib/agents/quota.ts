/**
 * Where each agent's live quota comes from, and how its own response shape
 * reads into the one meter the header shows. Every runtime reports its quota
 * differently, but a meter is a meter: a number between 0 and 100, an optional
 * reset instant and an optional label. The per-agent readers only locate those
 * three values; clamping and shaping happen once.
 */
...
export function quotaSourceFor(kind: AgentKind): QuotaSource {
  return QUOTA_SOURCES[kind]
}
