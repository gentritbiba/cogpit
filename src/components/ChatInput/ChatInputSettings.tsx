...
import { Bot, Check, ChevronDown, Code2, GitBranch, Plug, RefreshCw, Settings2, Shield, X, Zap } from "lucide-react"
import {
  cn,
  getEffortOptions,
...
export const ChatInputSettings = memo(function ChatInputSettings({
  ...
  if (mobile) {
    ...
    const permissionOptions = agentKind === "codex"
      ? CODEX_PERMISSION_MODES
      : CLAUDE_PERMISSION_MODES.filter((option) => option.value !== "auto" || autoModeAvailable)
    ...
    const changeMobilePermission = (nextMode: PermissionMode) => {
      if (nextMode === "bypassPermissions" && permissionMode !== "bypassPermissions") {
        setMobileOpen(false)
        setMobilePendingPermission(nextMode)
        return
      }
    ...
    return (
      <>
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 py-4">
              <section aria-labelledby="mobile-model-controls" className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {onAgentKindChange && (
                    <MobileControl label="Agent">
                      <select
                        aria-label="Agent"
                        value={agentKind}
                        onChange={(event) => onAgentKindChange(event.target.value as AgentKind)}
                  ...
                  {onPermissionModeChange && permissionMode && (
                    <MobileControl label="Access">
                      <select
                        aria-label="Access policy"
                        value={permissionMode}
                        onChange={(event) => changeMobilePermission(event.target.value as PermissionMode)}
                        className={MOBILE_SELECT_CLASS}
                      >
                      ...
                      <MobileControl label="Connections" wide>
                        <div className="flex flex-col gap-1">
                          ...
                          {(mcpServers ?? []).map((server) => {
                            const selected = selectedMcpServers?.includes(server.name) ?? false
                            ...
                            return (
                              ...
                              >
                                <span className={cn("size-2 shrink-0 rounded-full", connected && selected ? "bg-emerald-500" : connected ? "bg-muted-foreground" : "bg-amber-500")} />
        ...
        <Dialog
          open={mobilePendingPermission === "bypassPermissions"}
          onOpenChange={(open) => { if (!open) setMobilePendingPermission(null) }}
        >
          <DialogContent className="border-red-900/40 sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-red-300">Enable full access?</DialogTitle>
            ...
            <DialogFooter>
              <Button type="button" variant="ghost" onClick={() => setMobilePendingPermission(null)}>
                Cancel
              </Button>
              <Button
                type="button"
                ...
                onClick={() => {
                  ...
                  setMobilePendingPermission(null)
                }}
              >
                Enable full access
              </Button>
