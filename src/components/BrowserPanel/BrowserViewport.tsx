 ...
 * every pointer and key back into the page's own coordinates. The frame's
 * bitmap belongs to the socket hook, which closes it as soon as a newer frame
 * lands, so nothing here holds on to one.
 */
...
export function BrowserViewport({ frame, live, send, onSizeChange, className }: BrowserViewportProps) {
  ...
  const painted = live ? frame : null
  const deviceWidth = painted ? painted.header.deviceWidth : 0
  const deviceHeight = painted ? painted.header.deviceHeight : 0
  const fit = useMemo(
  ...
  useEffect(() => {
    ...
    if (!painted || fit.width <= 0 || fit.height <= 0) return
    if (painted.bitmap) {
      context.drawImage(painted.bitmap, fit.x, fit.y, fit.width, fit.height)
      return
    }
    if (!painted.blobUrl) return
    let cancelled = false
    const image = new Image()
    image.onload = () => {
      if (!cancelled) context.drawImage(image, fit.x, fit.y, fit.width, fit.height)
    }
    image.src = painted.blobUrl
    return () => {
      cancelled = true
    }
  }, [painted, fit, size])
  ...
  }, [live, send, fit, deviceWidth, deviceHeight])
